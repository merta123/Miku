    ...
    if (args.join("").includes("soundcloud")) {
      message.reply({
        embeds: [
          ...
          .setTitle(`${emoji.msg.search} Searching for your Request on <:soundcloud:909669943908384788> Soundcloud`)
      ...
      message.reply({
        embeds: [
          new MessageEmbed().setColor(es.color)
          .setTitle(`${emoji.msg.search} Searching for your Request on <:spotify:909669866787733554> Spotify`)
          .setDescription(`\`\`\`${String(args.join(" ")).substr(0, 2000)}\`\`\``)
        ]
      })
      ...
      message.reply({
        embeds: [
          new MessageEmbed().setColor(es.color)
          .setTitle(`${emoji.msg.search} Searching for your Request on <a:yt:883639024680583188> Youtube`)
          .setDescription(`\`\`\`${String(args.join(" ")).substr(0, 2000)}\`\`\``)
        ]
      })
      //play from YOUTUBE
